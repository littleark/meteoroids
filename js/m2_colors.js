		...
		*/
		...
		function init() {
			...
			for(var i=0;i<data.length;i++) {
				...
				//var	vel=new Vector2((2.5+(-0.5 + Math.random()*2))*(Math.round(Math.random())*2 -1),1+Math.random()),
				var  	vel=new Vector2((2.5+(-0.5 + Math.random()*2)),1+Math.random()),
				     	angle=vel.angle(),
				...
				particle.year=d.y;
				particle.gravity=0.001;//gravity;
